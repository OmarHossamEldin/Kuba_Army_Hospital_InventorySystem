...
<div class="salesInput">
        @csrf
        @if(count($lastinput)>0)
            <input type="text" id='id' value='{{$lastinput[0]->id+1}}' readonly placeholder="الرقم المسلسل للإدخال">
        @else
            <input type="text" id='id' value='{{1}}' readonly placeholder="الرقم المسلسل للإدخال">
        @endif
        <input type="number" id='id' class='col-4 vertical armynumber'   placeholder="رقم السند العسكري"><br>
        ...
        <input type="submit" class="btn btn-info addtobill vertical" value="إضافة للإدخال">
        </div>
    <hr>
    ...
    <a href="/dashboard" class="btn btn-danger cancleBill">ألغاء الإدخال</a>
    <input type="submit" class="btn btn-info printBill vertical" value="تسجيل الإدخال">
@endsection
